	...
	return function( elems ) {
		...
		for ( i = 0; ( elem = elems[ i ] ) != null; i++ ) {
			try {
				...
				// Only trigger close when necessary to save time
...
$.widget = function( name, base, prototype ) {
	...
	constructor.prototype = $.widget.extend( basePrototype, {
		...
		// TODO: close support for widgetEventPrefix
...
$.Widget.prototype = {
	...
	destroy: function() {
		...
		// We can probably close the unbind calls in 2.0
...
//>>label: uniqueId
//>>group: Core
//>>description: Functions to generate and close uniqueId's
	...
	stop: function( event, ui, draggable ) {
		...
		$.each( draggable.sortables, function() {
			...
			if ( sortable.isOver ) {
				...
				// However, don't set the draggable to close the helper
	...
	drag: function( event, ui, draggable ) {
		$.each( draggable.sortables, function() {
			...
			if ( innermostIntersecting ) {
				...
				// we fake the drag stop of the sortable, but make sure it doesn't close
...
var widgetsSortable = $.widget( "ui.sortable", $.ui.mouse, {
	...
	cancel: function() {
		...
		if ( this.placeholder ) {
			...
			//$(this.placeholder[0]).close(); would have been the jQuery way - unfortunately,
...
var widgetsAccordion = $.widget( "ui.accordion", {
	...
	_toggle: function( data ) {
		...
		// if we're switching panels, close the old header from the tab order
		// if we're opening from collapsed state, close the previous header from the tab order
...
$.widget( "ui.button", {
	...
	_updateIcon: function( option, value ) {
		...
		if ( !this.icon ) {
			...
			// If we are updating the icon close the old icon class
		...
		// If the icon is on top or bottom we need to add the ui-widget-icon-block class and close
		...
		if ( displayBlock ) {
			...
			// Position is beginning or end so close the ui-widget-icon-block class and add the
...
$.widget( "ui.dialog", {
	...
	_createButtons: function() {
		...
		// If we already have a button pane, close it
...
var widgetsSelectmenu = $.widget( "ui.selectmenu", [ $.ui.formResetMixin, {
	...
	_toggleAttr: function() {
		...
		// we always close classes first and add them second, otherwise if both classes have the
	...
	_resizeButton: function() {
		...
		// For `width: false`, just close inline style and stop
		if ( width === false ) {
...
$.widget( "ui.tabs", {
	...
	_toggle: function( event, eventData ) {
		...
		// If we're switching tabs, close the old tab from the tab order.
		// If we're opening from collapsed state, close the previous tab from the tab order.
...
$.widget( "ui.tooltip", {
	...
	_registerCloseHandlers: function( event, target ) {
		...
		// Only bind close handler for delegated targets. Non-delegated
	...
	close: function( event ) {
		...
		if ( !tooltipData ) {
			...
			// tooltip doesn't have full data, we always close ui-tooltip-open in case we're in
		...
		// Remove 'close' binding only on delegated targets
		if ( target[ 0 ] !== this.element[ 0 ] ) {
...
( function( jQuery, undefined ) {
	...
	// A set of RE_FOR_PARTS_TO_IGNORE's that can match strings and generate color tuples.
	stringParsers = [ {
