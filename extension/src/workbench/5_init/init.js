...
import authorize from './init/auth'
import showLicense from './init/showLicense'
...
function initWorkbench() {
   ...
   _.delay(() => {
      ...
      authorize().then(() => {
         ...
         if (localStorage.acceptedLicense) {
            ...
            showLicense().then(_initWorkbenchModules)
   ...
   function _initWorkbenchModules() {
      ...
      if (!localStorage['cth-one-time-notice-13/11/2017']) {

         localStorage['cth-one-time-notice-13/11/2017'] = true
         notifier.info("GTT Booster update 13/11/2017 - Automatic hiding of duplicate glossary entries has been disabled for now to prevent any potential issues. Updates may be added later to tweak the final version of Google's latest Terminology feature. Thank you. -Eirik Birkeland", 25000)
